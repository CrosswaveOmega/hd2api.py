...
class Planet(BaseApiModel, HealthMixin):
    ...
    health: Optional[int] = Field(
        alias="health", default=0, description="The current planet this planet has."
    )
    ...
    def __sub__(self, other: "Planet") -> "Planet":
        ...
        if self.regions is not None and other.regions is not None:
            new_regions = []
            other_regions_dict = {
                region.regionIndex: region for region in other.regions
            }
    ...
    def estimate_remaining_lib_time(self, diff: "Planet") -> str:
        ...
        if change == 0:
            if self.currentOwner.lower() != "humans":
                return "Stalemate."
            return ""
        timeval_base = self.calculate_timedelta_to_liberate(
            change, change > 0, offset=0
        )
        total_offset = 0
